...
private:
    ...
    void solveMPCA(void);
    void getCmd(void);
    void revVelCallBack(diablo_sdk::OSDK_LEGMOTORSConstPtr msg);
    double calcuEuler(double w, double x, double y, double z){
        double siny_cosp = 2*(w*z + x*y);
        double cosy_cosp = 1-2*(y*y +z*z);
        double yaws = std::atan2(siny_cosp, cosy_cosp);
        return yaws;
    }
