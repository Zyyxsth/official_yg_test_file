...
void publishFirstGoal() {
    ...
    if (!move_base_goal_queue_.empty()) {
        ...
        // 0.1.1
        is_allow_to_start = 1;
        std::cout<<"------------------is_allow_to_start-------------"<<std::endl;
    }
}
...
void ConvertPipeNode::userInputGoalPointsCallback(const ros::TimerEvent& event){
    ROS_WARNING("[convert node]: is_allow_to_start == %d",is_allow_to_start);

    // 0.1.1版本之后，直接开始重规划
    if (is_allow_to_start == 1)
    {
        if ((cmd_v(0) == 0) && (cmd_v(1) == 0) && (cmd_v(2) == 0) && (last_traj_t_rest_global < 1))  //-----------------------------------------------------------
        {
