...
int main(int argc, char **argv)
{
    ...
    diablo_imu_publisher imuPublisher(n,&vehicle);
    imuPublisher.imu_pub_init();

    diablo_battery_publisher batteryPublisher(n,&vehicle);
    batteryPublisher.battery_pub_init();

	diablo_motors_publisher motorsPublisher(n,&vehicle);
    motorsPublisher.motors_pub_init();

    diablo_body_state_publisher bodyStatePublisher(n,&vehicle);
    ...
    diabloCtrlNode com_Node("com",n);
    com_Node.pMovementCtrl = vehicle.movement_ctrl;
    com_Node.pTelemetry = vehicle.telemetry;
    com_Node.run_();
