...
void diablo_motors_publisher::motors_pub_init(void)
{
    ...
    timer_ = this->node_ptr.createTimer(ros::Duration(0.02),std::bind(&diablo_motors_publisher::lazyMotorsPublisher, this));
...
diablo_motors_publisher::diablo_motors_publisher(ros::NodeHandle& node,DIABLO::OSDK::Vehicle* vehicle)
:node_ptr(node)
{
