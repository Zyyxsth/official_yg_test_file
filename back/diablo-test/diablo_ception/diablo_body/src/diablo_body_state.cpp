...
void diablo_body_state_publisher::body_pub_init(void)
{
    ...
    timer_ = this->node_ptr.createTimer(ros::Duration(0.1),std::bind(&diablo_body_state_publisher::lazyPublisher, this));
...
diablo_body_state_publisher::diablo_body_state_publisher(ros::NodeHandle& node,DIABLO::OSDK::Vehicle* vehicle)
: node_ptr(node)
{
