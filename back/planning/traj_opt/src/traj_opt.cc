...
TrajOpt::TrajOpt(ros::NodeHandle& nh) : nh_(nh) {
  ...
  std::cout << "tracking_dur_ = " << tracking_dur_ << std::endl;
  nh.getParam("tracking_dist", tracking_dist_);
  std::cout << "tracking_dist_ = " << tracking_dist_ << std::endl;
...
void TrajOpt::addTimeIntPenalty(double& cost) {
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int j = 0; j < innerLoop; ++j) {
      ...
      //---------zan shi xiu gai , qu diao z fang xiang shang de tui li
      ...
      // if (grad_cost_p_corridor_not_z(pos, hPoly, grad_tmp, cost_tmp)) {
      //   gradViolaPc = beta0 * grad_tmp.transpose();
      //   gradViolaPt = alpha * grad_tmp.transpose() * vel;
      //   jerkOpt_.gdC.block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
      //   jerkOpt_.gdT(i) += omg * (cost_tmp / K_ + step * gradViolaPt);
      ...
      if (grad_cost_height(pos, grad_tmp, cost_tmp)) {
        gradViolaPc = beta0 * grad_tmp.transpose();
        jerkOpt_.gdC.block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
        jerkOpt_.gdT(i) += omg * (cost_tmp / K_ + step * grad_tmp.dot(vel));
        cost += omg * step * cost_tmp;
    }
...
bool TrajOpt::grad_cost_p_corridor_not_z(const Eigen::Vector3d& p,
                                        const Eigen::MatrixXd& hPoly,
                                        Eigen::Vector3d& gradp,
                                        double& costp) {
  bool ret = false;
  ...
  for (int i = 0; i < hPoly.cols(); ++i) {
    ...
    if (pen > 0) {
      double pen2 = pen * pen;
      // ✅ 只保留 x, y 分量的推力，z 方向不再受 corridor 约束
      gradp.head<2>() += rhoP_ * 3 * pen2 * norm_vec.head<2>();
      costp += rhoP_ * pen2 * pen;
      ret = true;
    }
  }
...
bool TrajOpt::grad_cost_height(const Eigen::Vector3d& pos,
  Eigen::Vector3d& grad_height,
  double& cost_height) {
// 固定的目标高度
double fixed_height = -0.2;   // 可以改成 astar_fixed_height_，并通过 rosparam 传进来
double dz = pos.z() - fixed_height;
...
// 如果偏离高度，就产生代价和梯度
if (fabs(dz) > 1e-6) {
double pen2 = dz * dz;           // 二次项代价
double pen3 = pen2 * fabs(dz);   // 三次项（让梯度更平滑）

grad_height.z() = 6.0 * pen2 * dz;   // 高度方向的梯度
cost_height = pen3;                  // 代价

grad_height *= rhoTracking_;         // 用现有的系数（也可以单独加 rhoHeight_）
cost_height *= rhoTracking_;
