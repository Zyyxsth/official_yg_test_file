...
class Env {
 ...
 private:
  ...
  double virtual_ceiling_height_ = 0.3;
  double astar_fixed_height_;
  double corridor_height_margin_;
 ...
 public:
  Env(ros::NodeHandle& nh,
    ...
    // 新增日志：验证发布器是否初始化成功
    if (hPolyPub_) {
      ROS_INFO("hPolyPub_ initialized successfully! Topic: /planning/sfc_visualization");
    } else {
      ROS_ERROR("hPolyPub_ initialization FAILED!");  // 若打印此日志，说明初始化失败
    }
    nh.getParam("tracking_dist", desired_dist_);
    nh.getParam("tolerance_d", tolerance_d_);
    nh.getParam("theta_clearance", theta_clearance_);
    nh.getParam("virtual_ceiling_height", virtual_ceiling_height_);
    nh.param("astar_fixed_height", astar_fixed_height_, 0.65); 
    nh.param("corridor_height_margin", corridor_height_margin_, 0.3); 
  ...
  void generateOneCorridor(const std::pair<Eigen::Vector3d, Eigen::Vector3d>& l,
    ...
    // 新增：追加天花板约束的超平面
    // 超平面方程：0*x + 0*y + 1*z ≤ virtual_ceiling_height_
    // 格式：[法向量(nx, ny, nz), 平面点(p_x, p_y, p_z)]，满足 nx*p_x + ny*p_y + nz*p_z = 约束值
    // Hyperplane3D ceiling_hyperplane(Eigen::Vector3d(0, 0, 1),  // 法向量（指向z轴正方向）
    // Eigen::Vector3d(0, 0, virtual_ceiling_height_));  // 平面上的点
    // current_hyperplanes.push_back(ceiling_hyperplane);  // 添加到超平面列表
  ...
  void generateSFC(const std::vector<Eigen::Vector3d>& path,
    ...
    for (uint i = 0; i < decompPolys.size(); i++) {
      ...
      // // 新增：追加天花板约束的超平面（与generateOneCorridor逻辑一致）
      // Hyperplane3D ceiling_hyperplane(Eigen::Vector3d(0, 0, 1),
      ...
      //  current_hyperplanes = bounded_poly.hyperplanes();
      // // -------------------------------------------
    ...
    // std::cout << "hPolys_size  = " << hPolys.size() << std::endl;
   
    filterCorridor(hPolys);

    // std::cout << "hPolys_size2  = " << hPolys.size() << std::endl;
    // // check again
    Eigen::MatrixXd curIH;
    Eigen::Vector3d interior;
  ...
  inline void visCorridor(const std::vector<Eigen::MatrixXd>& hPolys) {
     // 新增：打印日志，确认函数被调用
    ROS_INFO("visCorridor called! hPolys size: %ld", hPolys.size());
    // std::cout << "hPolys_size3  = " << hPolys.size() << std::endl;
    vec_E<Polyhedron3D> decompPolys;
...
// 2D 4-connected
neighbors.emplace_back(Eigen::Vector3i( 1, 0, 0), 1.0);
neighbors.emplace_back(Eigen::Vector3i(-1, 0, 0), 1.0);
neighbors.emplace_back(Eigen::Vector3i( 0, 1, 0), 1.0);
neighbors.emplace_back(Eigen::Vector3i( 0,-1, 0), 1.0);
...
if (!curPtr->valid) {
visited_nodes_.clear();
std::cout << "start position invalid!" << std::endl;
return false;
}
curPtr->parent = nullptr;
curPtr->g = 0;
...
// 固定高度层（0.65 m 转为体素索引）
int fixed_z = mapPtr_->pos2idx(Eigen::Vector3d(0,0,astar_fixed_height_)).z();

while (visited_nodes_.size() < MAX_MEMORY) {
for (const auto& neighbor : neighbors) {
Eigen::Vector3i neighbor_idx = curPtr->idx + neighbor.first; // 必须用局部变量
neighbor_idx.z() = fixed_z;  // 保持在固定高度
...
return ret;
  ...
  inline bool astar_search(const Eigen::Vector3d& start_p,
    ...
    //----------------------------------------------------------------------------------------- 12.19
    // Eigen::Vector3d start_p_fix = start_p;
    // start_p_fix.z() = 0.35;
    // Eigen::Vector3d end_p_fix = end_p;
    // end_p_fix.z() = 0.35;
    Eigen::Vector3i start_idx = mapPtr_->pos2idx(start_p);
    ...
    for (const auto& id : idx_path) {
      Eigen::Vector3d pos = mapPtr_->idx2pos(id);
      pos.z() = astar_fixed_height_;  // 覆盖成手动指定的高度
      path.push_back(pos);
